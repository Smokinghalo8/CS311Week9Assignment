...
// You don't need to manually define PageProps, Next.js will handle it
// Just directly accept the params in your async function
const EditVideoPage = async ({ params }: { params: { slug: string } }) => {
  const { slug } = params

  // Fetch the video by its slug
  const video = await prisma.video.findUnique({
    where: { id: Number(slug) },  // Convert slug (string) to number for query
  })

  // If video doesn't exist, redirect
  if (!video) {
    redirect('/videos')
  }
